...
let horraireCollection = [] ; 
horraireCollection = (await request("https://api.pray.zone/v2/times/day.json?city=mecca&date=2021-12-19"));
init();
...
function init() {
  ...
  world = Globe()(globeContainer)
      ...
      return `
        <div class="card">
          ...
          <div class="container">
             ...
             <hr />
             <div class="card-spacer">هجري  :    ${horraireCollection.results.datetime[0].date.hijri}</div><br />
             <hr />
             <div class="card-spacer">الامساك  :    ${horraireCollection.results.datetime[0].times.Imsak}</div><br />
             <span >الشروق  :    ${horraireCollection.results.datetime[0].times.Sunrise}</span><br />
             <span >الفجر  :    ${horraireCollection.results.datetime[0].times.Fajr}</span><br />
             <span >الظهر  :    ${horraireCollection.results.datetime[0].times.Dhuhr}</span><br />
             <span >العصر  :    ${horraireCollection.results.datetime[0].times.Asr}</span><br />
             <span >الغروب  :    ${horraireCollection.results.datetime[0].times.Sunset}</span><br />
             <span >المغرب  :    ${horraireCollection.results.datetime[0].times.Maghrib}</span><br />
             <span >العشاء  :    ${horraireCollection.results.datetime[0].times.Isha}</span><br />
             <span >منتصف الليل  :    ${horraireCollection.results.datetime[0].times.Midnight}</span><br />
          </div>
        </div>
      `;
    })
    .onPolygonHover((hoverD) =>
      world
        .polygonAltitude((d) => (d === hoverD ? 0.21 : 0.06))
        .polygonCapColor((d) =>
          d === hoverD ? 'steelblue' : 'LightSeaGreen'
        )
    )
